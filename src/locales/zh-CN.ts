export default {
  ...
  settingTabs: {
    system: {
      title: "系统",
      description:
        "基础设置、下载器（Qbittorrent、Transmission）、媒体服务器（Emby、Jellyfin、Plex）",
    },
    directory: {
      title: "存储 & 目录",
    ...
    search: {
      title: "搜索 & 下载",
      description:
        "搜索数据源（TheMovieDb、豆瓣、Bangumi）、下载任务标签、搜索站点",
    },
    subscribe: {
      title: "订阅",
    ...
    notification: {
      title: "通知",
      description:
        "通知渠道（微信、Telegram、Slack、SynologyChat、VoceChat、WebPush）、消息发送范围",
    },
    words: {
      title: "词表",
      description:
        "自定义识别词、自定义制作组/字幕组、自定义占位符、文件整理屏蔽词",
    },
    about: {
      title: "关于",
      description: "软件版本",
  ...
  notification: {
    ...
    wechat: {
      ...
      proxy: "代理地址",
      proxyHint:
        "微信消息的转发代理地址，2022年6月20日后创建的自建应用才需要，不使用代理时需要保留默认值",
      token: "Token",
    ...
    telegram: {
      ...
      token: "Bot Token",
      tokenHint:
        "Telegram机器人token，格式：123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11",
      chatId: "Chat ID",
      chatIdHint: "接受消息通知的用户、群组或频道Chat ID",
      users: "用户白名单",
      usersHint:
        "可使用Telegram机器人的用户ID清单，多个用户用,分隔，不填写则所有用户都能使用",
      admins: "管理员白名单",
      adminsHint: "可使用管理菜单及命令的用户ID列表，多个ID使用,分隔",
      adminsPlaceholder: "用户ID列表，多个ID使用,分隔",
    ...
    slack: {
      ...
      oauthTokenHint:
        "Slack应用`OAuth & Permissions`页面中的`Bot User OAuth Token`",
  ...
  subscribe: {
    ...
    resetConfirm:
      "重置后 {name} 将恢复初始状态，已下载记录将被清除，未入库的内容将会重新下载，是否确认？",
  ...
  setting: {
    ...
    system: {
      ...
      basicSettings: {
        ...
        customizeWallpaperApi: {
          label: "自定义壁纸API地址",
          hint: "会获取API返回内容中所有允许的安全域名地址的图片，需要同步设置安全域名地址",
          rules: {
            required: "必填项；请输入自定义壁纸API",
          },
        },
        recognizeSource: {
          label: "识别数据源",
        ...
        apiToken: {
          ...
          rules: {
            required: "必填项；请输入API Token",
            min: "API Token不得低于16位",
          },
        },
        githubToken: {
          label: "Github Token",
      ...
      advancedSettings: {
        title: "高级设置",
        subtitle: "系统进阶设置，特殊情况下才需要调整",
        task: {
          saveSuccess: "高级设置保存成功",
        },
        tabs: {
          system: "系统",
          media: "媒体",
        ...
        network: {
          ...
          securityImageDomains: {
            title: "安全图片域名",
            subtitle: "允许缓存的图片域名白名单，用于控制可信任的图片来源",
            noData: "暂无安全域名",
            placeholder: "添加域名，如：image.tmdb.org",
          },
        },
        log: {
          debug: {
          ...
          logBackupCount: {
            ...
            rules: {
              required: "请输入日志文件最大备份数量",
              min: "日志文件最大备份数量必须大于等于1",
            },
          },
          logFileFormat: {
            label: "日志文件格式",
      ...
      downloader: {
        title: "下载器",
        subtitle: "只有默认下载器才会被默认使用。",
        task: {
          saveSuccess: "下载器设置保存成功",
          saveFailed: "下载器设置保存失败！",
          noDefaultNotice: "未设置默认下载器，已将【{name}】作为默认下载器",
        },
        options: {
      ...
      mediaServers: {
        title: "媒体服务器",
        subtitle: "所有启用的媒体服务器都会被使用。",
        task: {
          saveSuccess: "媒体服务器设置保存成功",
          saveFailed: "媒体服务器设置保存失败！",
        },
        options: {
          emby: "Emby",
          jellyfin: "Jellyfin",
    ...
    site: {
      ...
      enableLocalCookieCloudHint:
        "使用内建CookieCloud服务同步站点数据，服务地址为：http://localhost:3000/cookiecloud",
      serviceAddress: "服务地址",
      serviceAddressPlaceholder: "https://movie-pilot.org/cookiecloud",
      serviceAddressHint:
        "远端CookieCloud服务地址，格式：https://movie-pilot.org/cookiecloud",
      userKey: "用户KEY",
      ...
      autoSyncIntervalHint:
        "从CookieCloud服务器自动同步站点Cookie到MoviePilot的时间间隔",
      ...
      confirmResetHint:
        "删除所有站点数据并重新从CookieCloud同步，操作请先清空涉及站点的相关设置。",
      resetSites: "重置站点数据",
      resettingSites: "正在重置...",
      syncInterval: {
        hourly: "每小时",
    ...
    words: {
      ...
      releaseGroupsPlaceholder:
        "支持正则表达式，特殊字符需要\\转义，一行代表一个制作组/字幕组",
      releaseGroupsHint:
        "支持正则表达式，特殊字符需要\\转义，一行代表一个制作组/字幕组",
      ...
      customizationDesc:
        "添加自定义占位符识别正则，重命名格式中添加{customization}使用。",
      customizationPlaceholder:
        "支持正则表达式，特殊字符需要\\转义，多个匹配对象请换行分隔",
      customizationHint:
        "支持正则表达式，特殊字符需要\\转义，多个匹配对象请换行分隔",
    ...
    search: {
      ...
      recognizeSourceDesc:
        "默认使用TMDB。豆瓣识别中文作品通常更友好，但有些国外作品信息不完整。",
      themoviedb: "TheMovieDb",
      douban: "豆瓣",
      ...
      downloadUserHint:
        "使用Telegram、微信等搜索时是否自动下载，使用逗号分割，设置为 all 代表所有用户自动择优下载",
      multipleNameSearch: "多名称资源搜索",
      multipleNameSearchHint:
        "使用多个名称（中文、英文等）搜索站点资源并合并搜索结果，会增加站点访问频率",
    ...
    directory: {
      ...
      movieRenameFormatHint:
        "使用Jinja2语法，格式参考：https://jinja.palletsprojects.com/en/3.0.x/templates",
    ...
    rule: {
      ...
      customRuleCopyFailed:
        "自定义规则复制失败：可能是浏览器不支持或被用户阻止！",
      customRuleCopyError: "自定义规则复制失败！",
      ruleGroupCopySuccess: "优先级规则组已复制到剪贴板！",
      ruleGroupCopyFailed:
        "优先级规则组复制失败：可能是浏览器不支持或被用户阻止！",
  ...
  dialog: {
    ...
    subscribeShare: {
      ...
      description: "说明",
      descriptionHint:
        "填写关于该订阅的说明，订阅中的搜索词、识别词等将会默认包含在分享中",
      shareUser: "分享用户",
    ...
    pluginMarketSetting: {
      ...
      repoUrl: "插件仓库地址",
      repoPlaceholder:
        "格式：https://github.com/jxxghp/MoviePilot-Plugins/,https://github.com/xxxx/xxxxxx/",
  ...
  plugin: {
    ...
    confirmReset:
      "此操作将恢复插件 {name} 的默认设置，并清除所有相关数据，确定要继续吗？",
    ...
    cloneFeature: "插件分身功能",
    cloneDescription:
      "创建插件的独立副本，拥有独立的配置和数据，适用于多账号、测试环境等场景",
    suffix: "分身后缀",
    ...
    cloneNotice:
      "分身插件创建后默认为禁用状态，需要手动配置启用。分身后缀一旦确定无法修改。",
    createClone: "创建分身",
  ...
  profile: {
    ...
    saveFailedWithNameChange:
      "【{oldName}】更名【{newName}】，信息保存失败：{message}！",
    ...
    secretKeyTip:
      "如果您在使用二维码时遇到困难，请在您的应用程序中选择手动输入以上代码。",
  ...
  folder: {
    ...
    confirmDeleteFolder:
      '确定要删除文件夹 "{folderName}" 吗？文件夹中的插件将移回主列表。',
