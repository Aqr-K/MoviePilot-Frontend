export default {
  ...
  settingTabs: {
    ...
    auth: {
      title: '登录',
      description: '登录页上的第三方登录入口，由插件提供',
    },
    about: {
      title: '关于',
      description: '软件版本',
  ...
  storage: {
    ...
    custom: '自定义',
    instanceName: '实例名',
    instanceNameHint: "同一存储类型下唯一，与类型拼成存储令牌，如 u115{'@'}work",
    instanceNameInvalid: "实例名不能为空，且不能含空白或 : / \\ {'@'}",
    bareTokenTarget: '承接不带实例名的路径',
    bareTokenTargetHint: '本存储类型每份路径没写实例名时落到这里，每个类型只能有一份；与「默认存储」是两回事',
    addInstance: '添加实例',
  },
  auth: {
    entry: '登录入口',
    entries: '登录入口',
    entriesDesc: '登录页上账号密码之外的登录按钮，由插件提供，每条配置对应一个按钮。',
    noTypesTitle: '暂无可用的登录入口类型',
    noTypesDesc: '登录入口全部由插件提供，安装并启用提供登录入口的插件后即可在此配置。',
    noEntries: '还没有配置登录入口，用下方的按钮添加一个。',
    instanceName: '入口名称',
    instanceNameHint: '登录页上该按钮显示的文字，同一类型下不可重名',
    instanceNameRequired: '入口名称不能为空',
    enabled: '启用',
    noConfigForm: '该登录入口类型没有提供配置界面。',
    identityProvider: '身份绑定标识',
    identityProviderHint:
      "留空即按「类型{'@'}实例名」派生。只有两种情况需要填：承接旧入口留下的存量绑定，或改名之后想保住已有绑定。",
    identityProviderEffective: '当前生效的标识：{identity}',
    identityConflict: '标识 {identity} 已被同族另一条配置认领，两条都不会出现在登录页上。',
    identityConflictBadge: '标识冲突',
    identityConflictTitle: '以下登录入口因标识冲突而不生效',
    identityConflictDesc:
      '同一个身份绑定标识被多条配置认领时，这几条一律不产出登录入口——让其中一条胜出，等于把另一台服务器的账号并进它的身份空间。请把其中至多一条填成该取值，其余留空或另填。',
  },
  serviceConfig: {
    providerIssueTitle: '以下配置当前不生效',
    providerIssueDesc: '提供这些服务类型的插件不在场，配置仍留在库里，提供方恢复后即刻生效。',
    reasonNotInstalled: '提供方 {extension} 未安装，装回该插件即可恢复',
    reasonDisabled: '提供方 {extension} 已安装但未启用，启用该插件即可恢复',
    reasonStartFailed: '提供方 {extension} 已启用，但服务类型 {type} 没有登记成功，请查看该插件的日志',
    reasonUnknown: '提供方 {extension} 当前不可用',
    typeUnavailable: '该服务类型当前不可用',
    defaultTarget: '默认调用目标',
    defaultTargetHint: '调用未指定实例时选中的那一份，每族至多一个',
    singleInstanceOnly: '该类型只能配置一份',
    maskedSecretHint: '凭据已隐藏，留空不动即保留原值，填入新值才会覆盖',
    createFailed: '新增配置失败！',
    updateFailed: '更新配置失败！',
    deleteFailed: '删除配置失败！',
    defaultTargetFailed: '设置默认调用目标失败！',
    loadFailed: '读取配置失败！',
    saveSuccess: '配置已保存',
  },
  filterRules: {
  ...
  setting: {
    ...
    rule: {
      ...
      originTitle: '规则来源',
      originHint: '运行期规则集按「内置 < 插件 < 用户」合并，同名时用户自定义永远生效。',
      originGroupTitle: '规则组来源',
      originGroupHint: '规则组只有插件与用户两层，同名时用户配置的规则组永远生效。',
      originLayerBuiltin: '内置',
      originLayerPlugin: '插件',
      originLayerUser: '用户自定义',
      originShadowedTag: '覆盖{source}',
      originShadowedGroupHint: '该规则组名覆盖了{sources}的同名规则组；改名或删除后，被覆盖的规则组会重新生效。',
      originConflictFallbackTag: '插件 {plugins} 的同名声明已作废，当前回落{source}定义',
      originIneffectiveTitle: '当前不生效',
      originIneffectiveHint:
        '多个插件声明了同一个标识，宿主无从裁决谁对，双方声明一并作废；请让其中一方改标识或停用该插件，也可以直接新增同名的自定义规则来接管。',
      originIneffectiveTag: '已失效',
      originConflictPlugins: '涉及插件：{plugins}',
      originLoadFailed: '来源信息加载失败，不影响规则本身的编辑与保存。',
      originEmpty: '暂无来源信息',
    },
    scheduler: {
      title: '定时作业',
