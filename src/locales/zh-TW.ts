export default {
  ...
  storage: {
    ...
    custom: '自定義',
    instanceName: '實例名',
    instanceNameHint: "同一存儲類型下唯一，與類型拼成存儲令牌，如 u115{'@'}work",
    instanceNameInvalid: "實例名不能為空，且不能含空白或 : / \\ {'@'}",
  ...
  serviceConfig: {
    ...
    providerIssueDesc: '提供這些服務類型的插件不在場，配置仍留在庫裡，提供方恢復後即刻生效。',
    reasonNotInstalled: '提供方 {extension} 未安裝，裝回該插件即可恢復',
    reasonDisabled: '提供方 {extension} 已安裝但未啟用，啟用該插件即可恢復',
    reasonStartFailed: '提供方 {extension} 已啟用，但服務類型 {type} 沒有登記成功，請查看該插件的日誌',
    reasonUnknown: '提供方 {extension} 當前不可用',
    typeUnavailable: '該服務類型當前不可用',
    defaultTarget: '預設調用目標',
    defaultTargetHint: '調用未指定實例時選中的那一份，每族至多一個',
    singleInstanceOnly: '該類型只能配置一份',
    maskedSecretHint: '憑據已隱藏，留空不動即保留原值，填入新值才會覆蓋',
    createFailed: '新增配置失敗！',
    updateFailed: '更新配置失敗！',
    deleteFailed: '刪除配置失敗！',
    defaultTargetFailed: '設置預設調用目標失敗！',
    loadFailed: '讀取配置失敗！',
    saveSuccess: '配置已保存',
  },
  ...
  setting: {
    ...
    rule: {
      ...
      originTitle: '規則來源',
      originHint: '運行期規則集按「內置 < 插件 < 用戶」合併，同名時用戶自定義永遠生效。',
      originGroupTitle: '規則組來源',
      originGroupHint: '規則組只有插件與用戶兩層，同名時用戶配置的規則組永遠生效。',
      originLayerBuiltin: '內置',
      originLayerPlugin: '插件',
      originLayerUser: '用戶自定義',
      originShadowedTag: '覆蓋{source}',
      originShadowedGroupHint: '該規則組名覆蓋了{sources}的同名規則組；改名或刪除後，被覆蓋的規則組會重新生效。',
      originConflictFallbackTag: '插件 {plugins} 的同名聲明已作廢，當前回落{source}定義',
      originIneffectiveTitle: '當前不生效',
      originIneffectiveHint:
        '多個插件聲明了同一個標識，宿主無從裁決誰對，雙方聲明一併作廢；請讓其中一方改標識或停用該插件，也可以直接新增同名的自定義規則來接管。',
      originIneffectiveTag: '已失效',
      originConflictPlugins: '涉及插件：{plugins}',
      originLoadFailed: '來源信息加載失敗，不影響規則本身的編輯與保存。',
      originEmpty: '暫無來源信息',
    },
    scheduler: {
      title: '定時作業',
    ...
    metaParser: {
      title: '名稱解析順序',
      subtitle: '調整各解析環的執行次序與啟停',
      orderHint: '順序即語義：靠後的解析環可以覆蓋靠前的解析環已經識別出的欄位。拖動即時生效。',
      pinnedTag: '固定',
      pinnedHint: '由宿主固定執行，無法排序或停用',
      unconfiguredTag: '預設順序',
      ownerHost: '宿主內建',
      distributionBuiltin: '內置',
      distributionMarket: '插件市場',
      toggleLabel: '啟用{name}',
      noExtensionRing: '暫無擴充功能提供的解析環',
      orderSaveSuccess: '解析順序已保存',
    },
    subscribe: {
      basicSettings: '基礎設置',
